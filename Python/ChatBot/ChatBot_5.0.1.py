...
class query(object):
    ...
    def querent(self, trigger):
        ...
        if usr_input != "!endchat":
            for i in trigger:
                for l in range(len(trigger[i])):
                    if SequenceMatcher(None, usr_input, trigger[i][l]).ratio() > self.highest_matching:
                        self.highest_matching = SequenceMatcher(
                            None, usr_input, trigger[i][l]).ratio()
    ...
    def comparer(self):
        ...
        if self.query_locator not in self.special_categories:
            if self.highest_matching >= 0.75:
                return self.reply()
            elif self.highest_matching == -1:
                return self.reply()
            else:
